...
// The mark's bounding box inside the 512×512 viewBox (from the trace).
const MARK_BBOX = { x: 84, y: 107, w: 340, h: 278 };
const ART_ROWS = 11;
 ...
 *  braille cells (2×4 dots each) straight from the SVG path data. */
function generateBrailleMark(rows: number): { grid: string[]; cols: number } {
  const cols = Math.round((rows * 2 * MARK_BBOX.w) / MARK_BBOX.h / 2) * 2; // square dots
  ...
  const s =
    Math.min((cols * 2) / MARK_BBOX.w, (rows * 4) / MARK_BBOX.h) * 0.94;
  c.translate(
    (cols * 2 - MARK_BBOX.w * s) / 2 - MARK_BBOX.x * s,
    (rows * 4 - MARK_BBOX.h * s) / 2 - MARK_BBOX.y * s,
  );
  c.scale(s, s);
  c.fillStyle = "#ffffff";

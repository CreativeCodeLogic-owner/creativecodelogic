 ...
 */
export function parseBashRead(command) {
    let cmd = command.trim();
    if (!cmd || DISALLOWED.test(cmd))
        return null;
    // Allow a trailing simple filter that doesn't change which file was read.
    const pipeSplit = cmd.split("|");
    if (pipeSplit.length > 2)
        return null;
    if (pipeSplit.length === 2 && !/^\s*(head|tail|wc|grep\s+-c)\b[^|]*$/.test(pipeSplit[1]))
        return null;
    cmd = pipeSplit[0].trim();
    const piped = pipeSplit.length === 2;
    const tokens = cmd.split(/\s+/);
    const bin = tokens[0];
    const fileArgs = (args) => args.filter((a) => !a.startsWith("-") && !/^\d+(,\d+)?[pd]?$/.test(a));
    if (bin === "cat") {
...
//# sourceMappingURL=bash-path-parser.js.map
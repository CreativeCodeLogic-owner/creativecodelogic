...
export function classifyCommand(command) {
    ...
    if (/^(grep|rg|ag)\b.*(-r|-R|--recursive)\b/.test(stripped) || /^(grep|rg)\b[^|]*\s\.$/.test(stripped))
        return "grep_flood";
    if (/^(grep|rg)\b/.test(stripped) && !/\|/.test(stripped))
        return "grep_flood";
    if (/^(cat|head|tail|sed\s+-n|awk)\b/.test(stripped))
        return "file_print";
    if (/^git\s+(show|log\s+.*-p|diff)\b/.test(stripped))
        return "git_show";
    if (/^(npm\s+(test|run\s+test)|pnpm\s+(test|run\s+test)|yarn\s+test|npx\s+(jest|vitest)|jest|vitest|pytest|python\s+-m\s+pytest|go\s+test|cargo\s+test|phpunit|php\s+artisan\s+test|rspec|mvn\s+test|gradle\s+test|\.\/gradlew\s+test)\b/.test(stripped))
        return "test";
    if (/^(npm\s+run\s+build|pnpm\s+(run\s+)?build|yarn\s+build|tsc\b|npx\s+tsc|cargo\s+build|go\s+build|make\b|webpack|vite\s+build|mvn\s+package|gradle\s+build|\.\/gradlew\s+build)\b/.test(stripped))
        return "build";
    return "unknown";
}
function elide(kept, total, unit) {
    return `[... ${total - kept} ${unit} elided ...]`;
}
/** grep/rg floods: first K matches per file + per-file counts + totals. */
function condenseGrep(stdout, perFile = 3) {
...
function condenseGitShow(stdout) {
    ...
    let currentFile = "";
    const flush = () => {
        if (currentFile && hunkLines > 0)
            out.push(`  ${currentFile}: ${hunkLines} diff lines (see full log)`);
        hunkLines = 0;
    };
    for (const line of lines) {
...
function condenseGeneric(stdout, head = 80, tail = 30) {
    const lines = stdout.split("\n");
 ...
 * looking (#82).
 */
export function condenseOutput(family, stdout, thresholdTokens, logPath) {
    ...
    if (condensedTokens > originalTokens * 0.7)
        return null; // not worth the change
    return { text, original_tokens: originalTokens, condensed_tokens: condensedTokens };
}
//# sourceMappingURL=bash-output-governor.js.map
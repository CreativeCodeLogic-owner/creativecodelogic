...
import { getWolfDir, ensureWolfDir, readJSON, readStdin, normalizePath, getProjectDir, emitHookJSON, recordInjection, hookMain, getSessionFilePath, projectRelativePath } from "./shared.js";
...
function skeletonHintsEnabled(wolfDir) {
    const cfg = readJSON(path.join(wolfDir, "config.json"), {});
...
async function main() {
    ...
    // prefix is not a path boundary, so sibling dirs (`../project-private/`) and
    // absolute externals used to be recorded in project-scoped state.
    ...
    if (relToProject.startsWith(".wolf/")) {
        ...
        try {
            ...
            if (!isRanged && (base === "anatomy.md" || base === "cerebrum.md" || base === "memory.md")) {
                let sizeTokens = 0;
                try {
                    sizeTokens = Math.ceil(fs.statSync(filePath).size / 3.5);
                }
                catch { }
                if (sizeTokens > 1500) {
                    // Locked: the once-per-file-per-session guard is only a guard if the
                    // flag is read and set in one serialized transaction (#83).
                    let advice = null;
                    mutateJSON(sessionFile, {
                        session_id: "", files_read: {}, anatomy_hits: 0, anatomy_misses: 0, repeated_reads_warned: 0,
                    }, HOOK_LOCK_BUDGET_MS, (session) => {
                        const warned = (session.wolf_read_advised ?? {});
                        if (warned[base])
                            return;
                        warned[base] = true;
                    ...
                    if (advice !== null) {
                        emitHookJSON("PreToolUse", { additionalContext: advice });
                    }
                }
            }
        }
    ...
    const notes = [];
    let denyReason = null;
    // ONE serialized transaction over the session file. Parallel Read calls are
    // ordinary Claude Code behavior and each fires its own hook process; an
    // unlocked read-modify-write silently lost 34 of 60 concurrent updates
    // (#83). The read happens inside the lock, so every decision below (already
    // read, one-shot denial, hit/miss counters) sees current on-disk state
    // rather than a snapshot another process has already superseded.
    // Model-visible output is emitted after the lock is released.
    mutateJSON(sessionFile, {
        ...
        if (session.files_read[normalizedFile] && session.files_read[normalizedFile].ranged !== true) {
            ...
            if (!modifiedSinceRead) {
                ...
                if (notes.length > 0)
                    recordInjection(session, "dup_warn", notes.join("\n"));
                return;
            }
            ...
            if (entry.symbols && entry.symbols.length > 0) {
                ...
                if (fresh) {
                    ...
                    if (entry.skeleton && entry.tokens > 2000 && skeletonHintsEnabled(wolfDir)) {
                        notes.push(`Signature outline of ${entry.file} (~${entry.tokens} tok full; symbols with line ranges follow):\n${entry.skeleton}\nRead with offset/limit to fetch just the part you need.`);
                    }
                    else {
        ...
        if (notes.length > 0)
            recordInjection(session, "anatomy_hint", notes.join("\n"));
    });
    if (denyReason !== null) {

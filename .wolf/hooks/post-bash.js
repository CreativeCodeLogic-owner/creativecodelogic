...
import { getWolfDir, ensureWolfDir, readJSON, readStdin, emitHookJSON, hookMain, getSessionFilePath, normalizePath, getProjectDir, projectRelativePath } from "./shared.js";
import { classifyCommand, condenseOutput, estimateTokens, DEFAULT_GOVERNOR_CONFIG } from "./bash-output-governor.js";
...
// ─────────────────────────────────────────────────────────────────────────────
function governorConfig(wolfDir) {
...
async function main() {
            ...
            if (relToProject !== null && relToProject !== "" && !relToProject.startsWith(".wolf/")) {
                ...
                mutateJSON(sessionFile, {}, HOOK_LOCK_BUDGET_MS, (session) => {
                    ...
                    if (read.full && prev && prev.ranged !== true && unchanged && prev.tokens > 0) {
                        notes.push(`OpenWolf: ${path.basename(normalized)} was already fully output this session (~${prev.tokens} tok) and is unchanged on disk since.`);
                        prev.count++;
                    }
                    else if (read.full) {
    ...
    if (config.mode !== "off" && originalTokens >= config.threshold_tokens) {
        ...
        const action = config.families[family] ?? "suggest";
        if (action !== "off") {
            ...
            if (probe && result) {
                ...
                notes.push(`OpenWolf: that ${family.replace("_", " ")} output was ~${result.original_tokens.toLocaleString("en-US")} tokens and now sits in context for the rest of the session.${copyNote} Narrower variants (grep with a file filter, head/tail, git show --stat) produce the same information at a fraction of the cost.`);
            }
        }
    }
    if (notes.length > 0) {
 ...
 */
function preserveOutput(logDir, logPath, stdout) {
    ...
    if (Buffer.byteLength(stdout, "utf-8") > CACHE_MAX_BYTES)
        return false;
    try {

...
import { getWolfDir, ensureWolfDir, readJSON, readStdin, emitHookJSON, recordInjection, hookMain, getSessionFilePath } from "./shared.js";
import { mutateJSON, HOOK_LOCK_BUDGET_MS } from "./anatomy-lock.js";
import { shouldSuggestFilter } from "./bash-filter.js";
function filterMode(wolfDir) {
...
async function main() {
    ...
    if (!shouldSuggestFilter(command)) {
        return;
    }
    ...
    const note = `OpenWolf: "${family}" output can be very large and every line enters your context. ` +
    ...
    mutateJSON(sessionFile, {}, HOOK_LOCK_BUDGET_MS, (session) => {
        if (session.bash_filter_suggested?.[family])
            return;

...
- Mobile perf (2026-09-25 CPU profile, 4x throttle): the main-thread cost is SVG `getPointAtLength()` path sampling at mount (WorldCreative.tsx:107 ~2.1s, WorldLogic.tsx:47 ~0.9s), not bundle size. React.lazy of below-fold chapters made TBT worse (mount still inside the TBT window + extra ScrollTrigger refreshes); inlining CSS did nothing (FCP is JS-bound). Local Lighthouse TBT runs ~8x PSI's; compare steps locally, never publish local numbers.
- Deep links (/#process, /#contact) don't scroll on load: the app renders client-side after the browser's fragment scroll. Pre-existing.

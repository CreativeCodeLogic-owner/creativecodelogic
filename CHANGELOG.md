...
### Changed
- Header now carries a decorative **frieze of scattered triquetra variants** (`NavFrieze`) instead of a single logo — a fresh per-load scatter of the styled variant SVGs (`src/assets/triquetra-variants/`, loaded via `import.meta.glob`) placed across the bar in shuffled slots, sized 40–110px (bleeding past the 64px band, clipped by the header's `overflow-hidden`), rotated ±30°, at 0.06–0.18 opacity (capped 0.10 behind nav text). Purely decorative: aria-hidden, pointer-events-none, below the nav content. The bar content is links + CTA (+ hamburger on mobile); scroll-to-top is owned by the floating back-to-top.
